...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-6">
        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" id="product-form">
            ...
            {{-- Add hidden field for primary image --}}
            ...
            <div class="mb-6">
                ...
                <div class="mb-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4" id="existing-images">
                        ...
                        <div class="relative group bg-white rounded-lg border border-gray-200 p-2 shadow-sm" data-image-id="{{ $image->id }}">
                            ...
                            <div class="absolute top-1 left-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                <button type="button"
                                    onclick="setAsPrimary({{ $image->id }})" // THIS IS NO ERROR
                                    class="bg-blue-500 text-white text-xs px-2 py-1 rounded-full font-medium hover:bg-blue-600">
                                    Set Primary
                                </button>
                            </div>
                            @endif
                            ...
                            {{-- Remove Button --}}
                            <button type="button"
                                onclick="removeExistingImage({{ $image->id }})" // THIS IS NO ERROR
                                class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                ×
                            </button>
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors duration-200">
                    ...
                    <label for="images" class="cursor-pointer">
                        ...
                        <div class="mt-2">
                            <span class="text-blue-600 font-medium">Click to upload additional images</span>
                ...
                {{-- New Images Preview --}}
                <div id="new-images-preview" class="mt-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 hidden">
...
<script>
    ...
    function removeExistingImage(imageId) {
        ...
        const currentValue = removedInput.value ? removedInput.value.split(',') : [];

        if (!currentValue.includes(imageId.toString())) {
            currentValue.push(imageId.toString());
            removedInput.value = currentValue.join(',');
        }
    ...
    // Add to the scripts section in edit.blade.php
    // FIXED JavaScript in resources/views/admin/products/edit.blade.php
    // Update the setAsPrimary function in edit.blade.php
    function setAsPrimary(imageId) {
        if (confirm('Set this image as the primary product image?')) {
            // Update the hidden field
            document.getElementById('primary_image_id').value = imageId;

            // Visual feedback - you can add some temporary UI indication
            const allImages = document.querySelectorAll('[data-image-id]');
            allImages.forEach(img => {
                const primaryBadge = img.querySelector('.bg-green-500');
                const setPrimaryBtn = img.querySelector('.bg-blue-500');

                if (img.dataset.imageId == imageId) {
                    // Show primary badge on selected image
                    if (setPrimaryBtn) {
                        setPrimaryBtn.style.display = 'none';
                    }
                    if (!primaryBadge) {
                        img.querySelector('.absolute.top-1.left-1').innerHTML =
                            '<span class="bg-green-500 text-white text-xs px-2 py-1 rounded-full font-medium">Primary</span>';
                    }
                } else {
                    // Show set primary button on other images
                    if (primaryBadge) {
                        primaryBadge.parentElement.innerHTML =
                            '<button type="button" onclick="setAsPrimary(' + img.dataset.imageId + ')" class="bg-blue-500 text-white text-xs px-2 py-1 rounded-full font-medium hover:bg-blue-600">Set Primary</button>';
                    }
                }
            });

            alert('Primary image selected. Click "Update Product" to save changes.');
        }
    }
</script>
@endpush
